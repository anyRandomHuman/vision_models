...
def predict_imgs_from_hdf5(path: Path, detector:Object_Detector, outpath, task, file='imgs.hdf5'):
    traj = path.name
    ...
    with time_record.open('w') as time_file:
        time_file.write(f'totel time: {total_time}\n img num: {num_imgs}\n avg:{total_time/(num_imgs)}')
    f.close()
...
if __name__ == "__main__":
    ...
    obj_det = detic.Detectron(to_tensor=False, classes=['banana', 'cup', 'carrot', 'saucepan', 'hair_dryer', 'bolt', 'hinge', 'bolt'])
